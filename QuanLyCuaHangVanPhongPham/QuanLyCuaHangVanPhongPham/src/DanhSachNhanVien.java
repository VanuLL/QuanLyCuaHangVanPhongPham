...
public class DanhSachNhanVien implements Doc {
	private Scanner sc = new Scanner(System.in);
	private int n;
	private NhanVien[] ds = new NhanVien[n];
	private NhanVien[] dsS = new NhanVien[n];
	private int x;
	...
	public NhanVien[] getDsS() {
		return dsS;
	}


	public void setDsS(NhanVien[] dsS) {
		this.dsS = dsS;
	}
	...
	public void xuatDSNV() {
		...
		System.out.println("");
		System.out.printf("%-15s%-15s%-15s%-15s%-15s%-15s\n","ID","Tên","Tuổi","Số điện thoại", "Giờ làm", "Lương" );
		for (int i = 0; i < 90;i++) {
			System.out.print("=");	
		}
		System.out.println("");
		...
		try {
	        ...
	        while(true) {
	        	...
	        	if (st == null) {
	        		...
	        		ds[ds.length-1] = new NhanVien(id, ten, tuoi, sdt, gioLam, luong);
	        	}
	        }
		...
		try {
	        ...
	        while(true) {
	        	...
	        	if (st == null) {
	        		...
	        		float gioLam = Float.parseFloat(s[4]);
	        		float luong = Float.parseFloat(s[5]);
	        		dsS[dsS.length-1] = new NhanVien(id, ten, tuoi, sdt, gioLam, luong);
	        	}
	        }
	        br.close();
	        fr.close();
